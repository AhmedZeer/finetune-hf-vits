...
class DataTrainingArguments:
    ...
    # NOTE: Since your dataset doesn't have speaker IDs, simply leave this as None.
    speaker_id_column_name: str = field(
        default=None,
        metadata={
            "help": "Name of the speaker id column if available. Leave as None if your dataset does not include speaker IDs."
        },
    )
    filter_on_speaker_id: int = field(
        ...
        metadata={
            "help": (
                "If `speaker_id_column_name` and `filter_on_speaker_id` are set, will filter the dataset to keep a single speaker_id."
            )
        },
    )
    ...
    uroman_path: str = field(
        ...
        metadata={
            "help": (
                ...
                "e.g the French checkpoint doesn't need it."
            )
        },
    )
...
class DataCollatorTTSWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # If a speaker ID is present in the sample, add it; otherwise, it will be None.
        batch["speaker_id"] = (
...
def main():
    ...
    # Check if a config file is passed and if it's YAML or JSON.
    if len(sys.argv) == 2 and (
        sys.argv[1].endswith(".json") or sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml")
    ):
    ...
    if training_args.do_train:
        raw_datasets["train"] = load_dataset(
            ...
            split=data_args.train_split_name+"[:10%]",
    ...
    # Since your dataset does not have speaker IDs, we ignore any speaker id column.
    # (If provided, you can also simply set speaker_id_column_name to None.)
    ...
    # Force only one speaker since no speaker IDs are present.
    if data_args.speaker_id_column_name is None:
    ...
    # Remove speaker id processing since dataset doesn't have it.
    # (No filtering or speaker mapping is performed here.)
    ...
    def prepare_dataset(batch):
        ...
        # No speaker_id processing since dataset doesn't have it.
        return batch
    ...
    with training_args.main_process_first(desc="apply_weight_norm"):
        ...
        # Speaker embedding override is skipped because speaker_id_column_name is None.
    ...
    for epoch in range(first_epoch, training_args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model, discriminator):
                # Build model inputs without speaker_id unless available.
                model_inputs = {
                ...
                if "speaker_id" in batch and batch["speaker_id"] is not None:
                    model_inputs["speaker_id"] = batch["speaker_id"]
            ...
            if do_eval:
                ...
                for step, batch in enumerate(eval_dataloader):
                    with torch.no_grad():
                        ...
                        if "speaker_id" in batch and batch["speaker_id"] is not None:
                            model_inputs["speaker_id"] = batch["speaker_id"]
                        model_outputs_train = model(**model_inputs)
                    ...
                    if accelerator.is_main_process:
                        # For full generation, only pass speaker_id if more than one speaker is configured.
                        if model.config.num_speakers > 1:
                            speaker_id = list(range(min(5, model.config.num_speakers)))
                            full_generation = model(**full_generation_sample.to(model.device), speaker_id=speaker_id)
                        else:
                            full_generation = model(**full_generation_sample.to(model.device))
